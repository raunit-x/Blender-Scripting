...
SUBDIVIDE = False  # set to True if you want to subdivide the mesh from 6890 -> 27554 vertices
...
# # load the UV map object
uv_object_filename = 'smpl_uv.obj'
bpy.ops.import_scene.obj(filepath=os.path.join(source_dir, 'Blender', uv_object_filename))  # import the object
uv_map_obj = bpy.data.objects['smpl_uv']


# DESELECT all the objects
select_or_deselect('DESELECT')


# Transfer UV map from 'smpl_uv' object to 'human_mesh_00001_sample' object
bpy.context.scene.objects.active = bpy.data.objects['smpl_uv']
bpy.data.objects['human_mesh_00001_sample'].select = True
bpy.ops.object.join_uvs()  # UV maps transfer from active to selected objects


# Now delete the smpl_uv object
select_or_deselect('DESELECT')
bpy.data.objects['smpl_uv'].select = True
bpy.ops.object.delete()
